...
class SansData(object):
    def __init__(self,data,metadata,q=None,qx=None,qy=None,theta=None):
    ...
    def __sub__(self,other):
        if isinstance(other,SansData):
            SansData(self.data-other.data,self.metadata,qx=copy(self.qx),qy=copy(self.qy),theta=copy(self.theta))
        else:
            return SansData(self.data-other,deepcopy(self.metadata),copy(qx=self.qx),qy=copy(self.qy),theta=copy(self.theta))
        
    def __rsub__(self, other):
        return SansData(other-self.data, deepcopy(self.metadata),qx=copy(self.qx),qy=self.qy,theta=self.theta)
...
def monitor_normalize(sansdata,mon0=1e8):
    ...
    return SansData(result,sansdata.metadata,qx=sansdata.qx,qy=sansdata.qy,theta=sansdata.theta)
...
def correct_blocked_beam(sample,blocked_beam,transmission):
    ...
    return SansData(result,sample.metadata,qx=sample.qx,qy=sample.qy,theta=sample.theta)
...
def correct_solid_angle(sansdata):
    ...
    sansdata_corrected=SansData(result.data,sansdata.metadata,qx=sansdata.qx,qy=sansdata.qy,theta=sansdata.theta)
    return sansdata_corrected 
...
def convert_q(sansdata):
    ...
    """
    L2=metadata['det.dis']
    x0=metadata['det.beamx']  #should be close to 64
    y0=metadata['det.beamy']  #should be close to 64
    wavelength=metadata['resolution.lmda']
    shape=sansdata.data.shape
    theta=np.array(shape,'Float64')
    q=np.array(shape,'Float64')
    qx=np.array(shape,'Float64')
    qy=np.array(shape,'Float64')
    ...
    for x in range(0,shape[0]):
        for y in range(0,shape[1]):
            ...
            q[x,y]=(4*np.pi/wavelength)*np.sin(theta)
            alpha=np.atan2(Y,X)
            qx[x,y]=q[x,y]*np.cos(alpha)
            qy[x,y]=q[x,y]*np.sin(alpha)
    return SansData(sansdata.data,deepcopy(sansdata.metadata),qx=qx,qy=qy,q=q,theta=theta)
...
def chain_corrections():
    ...
    #because transmission is a ratio, no need to normalize transmissions
    ...
    coord_left=(80,80)
    coord_right=(110,110)
    transmission_sample_cell_4m=generate_transmission(transmission_sample_cell_4m_q,transmission_empty_cell_4m_q,
                                                      coord_left,coord_right_q)
    print 'transmission=',transmission_4m
...
def map_files():
    ...
    filedict={'empty_1m':os.path.join(datadir,'SILIC001.SA3_SRK_S101'),
              ...
              'empty_cell_4m':os.path.join(datadir,'SILIC008.SA3_SRK_S1018'), 
              'sample_1m':os.path.join(datadir,'SILIC009.SA3_SRK_S109'),
              'sample_4m':os.path.join(datadir,'SILIC010.SA3_SRK_S010'),
