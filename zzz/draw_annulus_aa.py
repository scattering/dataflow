...
def annular_mask_antialiased(shape, center, inner_radius, outer_radius,
    ...
    oversampling: int (the mask is drawn on a canvas this many times bigger than the final size,
    then resampled down to give smoother edges)
    """
    intermediate_shape = (shape[0]*int(oversampling), shape[1]*int(oversampling))
    im = Image.new('F', intermediate_shape, color = background_value) # this is a 32-bit float image
