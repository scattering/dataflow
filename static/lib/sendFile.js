...
var sendFile = 20000000; // maximum allowed file size
...
sendFile = (function(toString, maxSize){
    ...
    return  function(handler){
        ...
        var extra_headers = handler.extra_headers || [];
        var extra_data = handler.extra_data || {};
        ...
        xhr.setRequestHeader('X-CSRFToken', handler.csrf_token);
        ...
        for (var i in extra_headers) {
            var header = extra_headers[i]
            xhr.setRequestHeader(header[0], header[1]);
        }
        var formData = new FormData();
        for (var i in extra_data) {
            formData.append(i, extra_data[i])
        }
        formData.append("FILES", handler.file);
        xhr.send(formData);
        return  handler;
    };
