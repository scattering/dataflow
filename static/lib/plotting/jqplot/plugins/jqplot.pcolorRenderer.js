...
(function($) {
     ...
     * 
     * > <script language="javascript" type="text/javascript" src="../src/plugins/jqplot.pcolorRenderer.js"></script>
     * 
     * Data must be supplied in one of these forms:
     * 
     * > [[x1, y1, z1], ...]
     * > [{ dims: { xmin: xmin, xmax: xmax, ymin: ymin, ymax: ymax } }, [z11, z12, z13], [z21, z22, z23], ...]
     * > [{ dims: { xmin: xmin, xmax: xmax, ymin: ymin, ymax: ymax } }, z1, z2, z3, z4, z5, ...]
     * 
     ...
     * 
     * > seriesDefaults: {
     * >     renderer: $.jqplot.pcolorRenderer,
     * >     rendererOptions: {
     * >         bubbleAlpha: 0.7,
     ...
     * 
     */
    $.jqplot.pcolorRenderer = function(){
        $.jqplot.LineRenderer.call(this);
    };
    
    $.jqplot.pcolorRenderer.prototype = new $.jqplot.LineRenderer();
    $.jqplot.pcolorRenderer.prototype.constructor = $.jqplot.pcolorRenderer;
    ...
    // Called with scope of a series.
    $.jqplot.pcolorRenderer.prototype.setGridData = function(plot) {
    ...
    // Called with scope of a series.
    $.jqplot.pcolorRenderer.prototype.makeGridData = function(data, plot) {
        // recalculate the grid data
        var xp = this._xaxis.series_u2p;
        var yp = this._yaxis.series_u2p;
        var gd = [];
    ...
    $.jqplot.pcolorRenderer.prototype.draw = function (ctx, gd, options) {
    ...
    function highlight (plot, sidx, pidx) {
        ...
        plot.plugins.pcolorRenderer.highlightedSeriesIndex = sidx;
        ...
        if (s.labels[pidx]) {
            plot.plugins.pcolorRenderer.highlightLabel = s.labels[pidx].clone();
    ...
    function handleMove(ev, gridpos, datapos, neighbor, plot) {
        if (neighbor) {
            ...
            if (plot.series[ins[0]].highlightMouseOver && !(ins[0] == plot.plugins.pcolorRenderer.highlightedSeriesIndex && ins[1] == plot.series[ins[0]]._highlightedPoint)) {
    ...
    function handleMouseDown(ev, gridpos, datapos, neighbor, plot) {
        if (neighbor) {
            ...
            var pi = neighbor.pointIndex;
            var ins = [si, pi, neighbor.data, plot.series[si].gridData[pi][2]];
            if (plot.series[ins[0]].highlightMouseDown && !(ins[0] == plot.plugins.pcolorRenderer.highlightedSeriesIndex && ins[1] == plot.series[ins[0]]._highlightedPoint)) {
                var evt = jQuery.Event('jqplotDataHighlight');
                evt.pageX = ev.pageX;
                evt.pageY = ev.pageY;
                plot.target.trigger(evt, ins);
    ...
    function handleMouseUp(ev, gridpos, datapos, neighbor, plot) {
        var idx = plot.plugins.pcolorRenderer.highlightedSeriesIndex;
    ...
    function postPlotDraw() {
        this.plugins.pcolorRenderer = {highlightedSeriesIndex:null};
        this.plugins.pcolorRenderer.highlightCanvas = new $.jqplot.GenericCanvas();
        this.plugins.pcolorRenderer.highlightLabel = null;
        this.plugins.pcolorRenderer.highlightLabelCanvas = $('<div style="position:absolute;"></div>');
        ...
        this.plugins.pcolorRenderer.highlightLabelCanvas.css({top:top, left:left, width:width+'px', height:height+'px'});

        this.eventCanvas._elem.before(this.plugins.pcolorRenderer.highlightCanvas.createElement(this._gridPadding, 'jqplot-pcolorRenderer-highlight-canvas', this._plotDimensions));
        this.eventCanvas._elem.before(this.plugins.pcolorRenderer.highlightLabelCanvas);
        
        var hctx = this.plugins.pcolorRenderer.highlightCanvas.setContext();
    }
    ...
    function preInit(target, data, options) {
        ...
        var setopts = false;
        if (options.seriesDefaults.renderer == $.jqplot.pcolorRenderer) {
            setopts = true;
        }
        else if (options.series) {
            for (var i=0; i < options.series.length; i++) {
                if (options.series[i].renderer == $.jqplot.pcolorRenderer) {
                    setopts = true;
                }
            }
        }
