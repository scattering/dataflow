...
def read_data(mydirectory,myfilebase,myend):
    ...
    for currfile in flist:
        ...
        Qx=np.concatenate((Qx,np.array(mydata.data['h'])))
        Qy=np.concatenate((Qy,np.array(mydata.data['k'])))
        Qz=np.concatenate((Qz,np.array(mydata.data['l'])))
        Counts=np.concatenate((Counts,np.array(mydata.data['counts'])*mon0/mon))
        Errs=np.concatenate((Errs,np.sqrt(np.array(mydata.data['counts']))*mon0/mon))
    return Qx,Qy,Counts, Errs
...
if 1:
    ...
    problem.output_path=r'L:\dataflow_fresh\tasbumps\TestBumpDir'
